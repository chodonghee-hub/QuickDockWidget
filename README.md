...
_Windows에서 글로벌 단축키(기본: Ctrl+\`)로 즐겨찾기 웹사이트를 빠르게 실행하는 북마크 위젯 앱_
...
이후 Windows 재시작 시 `QuickDock`이 자동으로 실행됩니다.
